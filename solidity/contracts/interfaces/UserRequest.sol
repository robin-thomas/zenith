...
abstract contract UserRequest is ReentrancyGuard, ConfirmedOwner {
    ...
    constructor(
        ...
        require(
            sxtRelayAddress != ISxTRelayProxy(ZERO_ADDRESS),
            "UserRequest: Cannot set to Zero Address"
        );
        require(
            chainlinkTokenAddress != LinkTokenInterface(ZERO_ADDRESS),
            "UserRequest: Cannot set to Zero Address"
        );
    ...
    modifier onlySxTRelay() {
        require(
            msg.sender == address(getSxTRelayContract().impl()),
            "UserRequest: Only callable by SxT Request Contract"
        );
        _;
    }
     ...
     */
    function withdrawChainlink(
        address to,
        uint256 amount
    ) external nonReentrant onlyOwner {
        bool transferResult = chainlinkToken.transfer(to, amount);
